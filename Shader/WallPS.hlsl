...
//Pixel Shader
float4 ps_wall(PS_INPUT input) : SV_TARGET0
{
    //元のテクスチャ色
    float4 texColor = g_SrcTexture.Sample(g_SrcSampler, input.Tex0);

    //ノーマルマップ
    float3 NormalTex = g_NormalTexture.Sample(g_SrcSampler, input.Tex0).xyz;
    ...
    float brightness = lerp(0.4f, 1.25f, light);
        ...
        input.ShadowMapPos.z >= 0 && input.ShadowMapPos.z <= 1)
    {
        ...
        float depth = g_ShadowMap0Texture.Sample(g_ShadowMap0Sampler, uv).r;
